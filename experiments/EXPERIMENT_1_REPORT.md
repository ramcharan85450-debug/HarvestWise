...
**A real (if currently inactive) leakage vector was found and is reported, not hidden.** `training/dataset.py::_impute_missing_soil()`, called inside `build_dataset_from_processed()` *before* this script's chronological split, fills any field's missing soil values using the mean of all other examples' soil vectors — pooled across the *entire* 21-example set, including whichever examples later land in val/test. Checked directly: **zero of the 21 real examples currently have missing soil** (confirmed by testing `soil_x` for NaN before any imputation could run), so this vector is not actually triggering any cross-split averaging right now. It is flagged here as a real risk present in the code path, which would need to be fixed (imputation mean computed from `train_examples` only) before it could be trusted if a future field's soil pull ever failed.
...
| Soil-imputation cross-split pooling | Present in code, **currently inactive** (0 of 21 examples need imputation) |
...
- **The soil-imputation leakage vector is currently inactive but present in the code** (§9) — worth fixing (imputation mean from `train_examples` only) before this experiment is extended to any field whose soil pull might fail.
